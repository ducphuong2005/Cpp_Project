...
//kiem tra co sinh vien nao ten la "An" khong
void CheckName (TRO &L){
	int d=0;
	TRO Q=L;
	while (Q!=NULL){
		if(strcmp(Q->infor.hoTen, "An")==0)
    ...
    if(d!=0)
     cout<<"Co sinh vien ten la An";
     else cout<<"Khong tim thay sinh vien ten la An";
}

//xoa sinh vien ten la "An"
void deleteName (TRO &L){
    TRO Q=L;
    TRO P;
    while (Q!=NULL){
        if(strcmp(Q->infor.hoTen, "An")==0){
...
int main()
{
    ...
    CheckName(L);
//    deleteName(L);
//    cout << "Danh sach sinh vien sau khi xoa sinh vien ten la An: " << endl;
//    outputTT(L);
    delAllName(L);
    cout << endl<<"Danh sach sinh vien sau khi xoa sinh vien ten la An: " << endl;
    outputTT(L);
    return 0;
}